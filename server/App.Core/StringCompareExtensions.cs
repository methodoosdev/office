...
namespace App.Core
{
    public static class StringCompareExtensions
    {
        ...
        public static bool Like(this string input, IEnumerable<string> patterns, bool ignoreCase = true)
        {
            if (patterns == null || input == null) return false;
            return patterns.Any(pattern => Like(input, pattern, ignoreCase));
        }

        public static bool IsLike(this string input, IEnumerable<string> patterns)
        {
            if (patterns == null || input == null) return false;
            return patterns.Any(pattern => IsLike(input, pattern));
        }
        ...
        public static bool ContainsIgnoreCase(this string source, string destination) 
        {
            ...
            return Regex.IsMatch(source, Regex.Escape(destination), RegexOptions.IgnoreCase);
        }
    }
}