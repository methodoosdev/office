...
{
    public class ListingF5RetrievePage : PageTest
    {
        //TaxisNet
        ...
        public ListingF5RetrievePage(string connectionId) : base(connectionId: connectionId, headless: false)
        {
        ...
        public async Task<IList<ListingF5Result>> Execute(string vat, int year, int month)
        {
            ...
            var listModel = new List<ListingF5Result>();
            ...
            foreach (var row in rows)
            {
                ...
                {
                    var element = cols[3].GetByText("Επεξεργασία");

                    if (await element.CountAsync() > 0)
                    {
                        await ClickToUrl(element, _displayDeclarationsListPage);

                        var listingInfos = new List<ListingF5Info>();
                        var rows1 = await _selectTableLocator.AllAsync();

                        foreach (var row1 in rows1)
                        {
                            var cols1 = await row1.Locator("> td").AllAsync();
                            var buttonView = cols1[7].GetByText("Προβολή");
                            if (await buttonView.CountAsync() > 0)
                            {
                                listingInfos.Add(new ListingF5Info { DateSubmit = await cols1[3].InnerTextAsync(), Button = buttonView });
                            }
                        }
        ...
        private IList<ListingF5Result> ExtractPdf(string pdfText, string info, string group)
        {
            var listModel = new List<ListingF5Result>();
