...
{
    public class KeaoRetrieveController : BaseController
    {
        [HttpPost]
        public async Task<IActionResult> List(int index, int traderId, string traderName, string userName, string password, string connectionId)
        {
            ...
            try
            {
                await using (var page = new KeaoRetrievePage(connectionId))
                {
                    var title = "ΕΦΚΑ Μη μισθωτών";
                    try
                    {
                        ...
                        {
                            result.AddError($"<b>Σύνδεση:</b> {traderName}. Σφάλμα: Μη έγκυροι κωδικοί ΚΕΑΟ");
                        }
                    }
