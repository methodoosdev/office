...
namespace App.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        public virtual IQueryable<Customer> Table => _customerRepository.Table;

        public virtual IQueryable<CustomerRole> CustomerRoleTable => _customerRoleRepository.Table;

        public virtual async Task<IList<Customer>> GetRegisterCustomersAsync()
        {
            var registeredRole = await GetCustomerRoleBySystemNameAsync(NopCustomerDefaults.RegisteredRoleName);

            var customerIds = (from registered in _customerRepository.Table
                               join ccm in _customerCustomerRoleMappingRepository.Table on registered.Id equals ccm.CustomerId
                               where ccm.CustomerRoleId == registeredRole.Id
                               select registered.Id).ToArray();

            return await GetCustomersByIdsAsync(customerIds);
        }
        ...
        public virtual IQueryable<Customer> GetAllCustomersByRoles(int[] customerRoleIds = null)
        {
        ...
        public virtual async Task<int> DeleteGuestCustomersAsync(DateTime? createdFromUtc, DateTime? createdToUtc, bool onlyWithoutShoppingCart)
        {
            ...
            var guestsToDelete = from guest in _customerRepository.Table
                                 ...
                                 where blogComment == null && newsComment == null &&
        ...
        /// The task result contains the result
        /// </returns>
        public virtual async Task<bool> IsOfficeAsync(Customer customer, bool onlyActiveCustomerRoles = true)
        {
            return await IsInCustomerRoleAsync(customer, NopCustomerDefaults.OfficesRoleName, onlyActiveCustomerRoles);
        }

        public virtual async Task<bool> IsEmployeeAsync(Customer customer, bool onlyActiveCustomerRoles = true)
        {
            return await IsInCustomerRoleAsync(customer, NopCustomerDefaults.EmployeesRoleName, onlyActiveCustomerRoles);
        }

        public virtual async Task<bool> IsTraderAsync(Customer customer, bool onlyActiveCustomerRoles = true)
        {
            return await IsInCustomerRoleAsync(customer, NopCustomerDefaults.TradersRoleName, onlyActiveCustomerRoles);
        }
