...
namespace App.Services.Customers
{
    ...
    public partial class CustomerRegistrationService : ICustomerRegistrationService
    {
        ...
        public virtual async Task<CustomerRegistrationResult> RegisterCustomerAsync(CustomerRegistrationRequest request)
        {
            ...
            {
                result.AddError(await _localizationService.GetResourceAsync("App.Errors.WrongEmail"));
                return result;
            }
        ...
        public virtual async Task<IActionResult> SignInCustomerAsync(Customer customer, string returnUrl, bool isPersist = false)
        {
            ...
            await _tokenStoreService.AddCustomerTokenAsync(customer, result.RefreshTokenSerial, result.AccessToken, null);

            _antiforgeryCookieService.RegenerateAntiForgeryCookies(result.Claims);

            //sign in new customer
            await _authenticationService.SignInAsync(customer);
            ...
            return new JsonResult(new { access_token = result.AccessToken, refresh_token = result.RefreshToken });
        }
