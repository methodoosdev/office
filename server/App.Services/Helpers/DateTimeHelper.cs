...
namespace App.Services.Helpers
{
    ...
    public partial class DateTimeHelper : IDateTimeHelper
    {
        ...
        #region new Methods
        public DateTime LastDateOfWeek(DateTime dt)
        {
            return FirstDateOfWeek(dt).AddDays(6);
        }
        public DateTime FirstDateOfWeek(DateTime dt)
        {
            int diff = (7 + (dt.DayOfWeek - DayOfWeek.Monday)) % 7;
            return dt.AddDays(-1 * diff).Date;
        }

        public DateTime FirstDateOfWeek(int week, int year)
        {
            week = week - 1;
            var firstDateOfYear = new DateTime(year, 1, 1, 0, 0, 0);
            var firstDateOfWeek = FirstDateOfWeek(firstDateOfYear);
            if (firstDateOfWeek.Year < year)
            {
                firstDateOfWeek = firstDateOfWeek.AddDays(7);
                week = week - 1;
            }

            return firstDateOfWeek.AddDays((week * 7));
        }

        public async Task<int> GetWeekOfYear(DateTime dt)
        {
            var language = await _workContext.GetWorkingLanguageAsync();
            var cultureInfo = new CultureInfo(language.LanguageCulture);

            var week = cultureInfo.Calendar.GetWeekOfYear(dt, CalendarWeekRule.FirstDay, DayOfWeek.Monday);
