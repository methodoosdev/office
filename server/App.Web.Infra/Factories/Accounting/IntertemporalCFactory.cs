...
{
    public partial interface IIntertemporalCFactory
    {
    ...
    public class IntertemporalCFactory : IIntertemporalCFactory
    {
        ...
        public IntertemporalCFactory(IFieldConfigService fieldConfigService, IAppDataProvider dataProvider, ILocalizationService localizationService, IWorkContext workContext)
        {
        ...
        public virtual async Task<IntertemporalSearchModel> PrepareIntertemporalSearchModelAsync(IntertemporalSearchModel searchModel)
        {
            ...
            {
                await _fieldConfigService.GetTradersMultiColumnComboBox<IntertemporalSearchModel>(nameof(IntertemporalSearchModel.TraderId), FieldConfigType.WithCategoryBookC)
            };
        ...
        public async Task<IList<IntertemporalData>> PrepareIntertemporalQueryListAsync(TraderConnectionResult connectionResult)
        {
            ...
            var results = await _dataProvider.QueryAsync<IntertemporalResult>(connectionResult.Connection, IntertemporalResultQuery.GetCCategory, pCompanyId, pSchema);
            ...
            foreach (var year in years)
            {
                ...
                model.Sales = sales;
                model.Income = income;
                model.OpeningInventory = openingInventory;
                model.Purchases = purchases;
                model.ClosingInventory = closingInventory;
                model.Expenses = expenses;

                model.GrossProfit = grossProfit;
                model.GrossProfitRate = sales == 0 ? 0 : Math.Round((grossProfit / sales) * 100, 2);

                model.CostOfGoodsSold = costOfGoodsSold;
                model.CostOfGoodsSoldRate = sales == 0 ? 0 : Math.Round((costOfGoodsSold / sales) * 100, 2);

                model.ExpensesRate = sales == 0 ? 0 : Math.Round((expenses / sales) * 100, 2);

                model.NetProfits = netProfits;
                model.NetProfitsRate = sales == 0 ? 0 : Math.Round((netProfits / sales) * 100, 2);
