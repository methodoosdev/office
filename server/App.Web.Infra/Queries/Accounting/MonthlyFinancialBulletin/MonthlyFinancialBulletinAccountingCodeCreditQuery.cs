...
{
    public class MonthlyFinancialBulletinAccountingCodeCreditQuery : TraderFactoryQuery
    {
        public MonthlyFinancialBulletinAccountingCodeCreditQuery(int logistikiProgramTypeId, int categoryBookTypeId) : base(logistikiProgramTypeId, categoryBookTypeId)
        {
        }
        ...
        public override string SoftOne_C => @"
            SELECT A.CODE AS Id, A.NAME AS Description, B.FISCPRD AS Year, B.PERIOD AS Periodos,
                A.ACNGRADE AS Level, 
                SUM(B.LCREDITTMP) - SUM(B.LDEBITTMP) AS Total
