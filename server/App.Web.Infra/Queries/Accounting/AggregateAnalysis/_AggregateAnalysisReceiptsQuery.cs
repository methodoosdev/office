...
{
    public class _AggregateAnalysisReceiptsQuery : TraderFactoryQuery
    {
        public _AggregateAnalysisReceiptsQuery(int logistikiProgramTypeId, int categoryBookTypeId) : base(logistikiProgramTypeId, categoryBookTypeId)
        {
        }

        public override string SoftOne_C => @"
            SELECT A.TRNDATE AS Date, 'receipts' AS Id, 
            ...
            WHERE A.COMPANY=@pCompanyId 
                AND A.SOSOURCE IN (1381, 1413) 
                AND A.SOREDIR=0 
	            AND A.TRNDATE >= @pStartDate
	            AND A.TRNDATE <= @pEndDate
                AND A.SODTYPE IN (13, 14)
