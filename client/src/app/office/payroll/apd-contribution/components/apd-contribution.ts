...
import { ApdContributionUnitOfWork } from "@officeNg";
import { TranslationService } from "@core";


@Component({
    selector: "apd-contribution",
    templateUrl: "./apd-contribution.html",
    animations: [stateAnimation],
})

export class ApdContributionComponent implements OnInit {
    ...
    constructor(
        private uow: ApdContributionUnitOfWork,
        private toastrService: ToastrService,
        private translationService: TranslationService) {

        this.title = this.translationService.translate('menu.apdContribution');
    ...
    isHidden(columnName: string): boolean {
        ...
        if ((month != 1 && month != 5))
            hiddenColumns = ['christmasPresentEfka', 'christmasPresentTeka', 'easterPresentEfka', 'easterPresentTeka'];
        else if (month == 1)
            hiddenColumns = ['easterPresentEfka', 'easterPresentTeka'];
        else if (month == 5)
            hiddenColumns = ['christmasPresentEfka', 'christmasPresentTeka'];
